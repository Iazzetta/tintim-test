...
class CourseService(CoreService):
    ...
    def _validate_grade(self, grade: int|str):
        if not isinstance(grade, int) and not isinstance(grade, str):
            raise InvalidGradeException("Grade must be a number or a string.")
        ...
        if isinstance(grade, str):
            if len(grade) != 2 and len(grade) != 1:
                raise InvalidGradeException("Grade must be 1 or 2 letters (A+, B, C-).")

            if grade not in ["A+", "A", "A-", "B+", "B", "B-", "C+", "C", "C-", "D", "F"]:
                raise InvalidGradeException("Grade must be one of the following options: A+, A, A-, B+, B, B-, C+, C, C-, D and F.")
